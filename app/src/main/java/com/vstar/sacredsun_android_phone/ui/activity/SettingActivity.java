        ...
        if(!TextUtils.isEmpty(result)) {
            if(result.matches("^[a-zA-Z0-9]+$")) {
                DialogInterface.OnClickListener workCenterListener = (dialog, which) -> {
                    SPHelper.putAndApply(SettingActivity.this, getString(R.string.FIRST_LAUNCH), true);
                    SPHelper.putAndApply(SettingActivity.this,getString(R.string.WORK_CENTER_CODE_SESSION),result);
